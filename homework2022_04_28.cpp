...
int main() {
    ...
    vector<bool> and_res = and_funciton(hash1, hash2), or_res = or_function(hash1, hash2);
    vector<int> arr_in = generate_array_from_hash(and_res, delta),
                arr_un = generate_array_from_hash(or_res, delta);
    cout << "Union:\n";
    cout_array(arr_un);
    cout << "Intersection:\n";
    cout_array(arr_in);
