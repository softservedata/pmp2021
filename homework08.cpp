// Порівняти вміст двох дійсних масивів. Створити hash функцію для дійсних чисел.
...
vector<bool> getHashArray(const vector<T> &arr, const T delta, const size_t hash_size, bool out = false) {
    ...
    if (out) {
        cout << "hash array:\n";
        for (auto &&el: hash) // function print_array not possible because of vector<bool> 1 bit optimization
            cout << el;
        cout << '\n';
    }
    return hash;
